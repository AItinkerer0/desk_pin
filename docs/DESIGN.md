...
### Phase 3 — 모션 위젯 탐사와 최종 구성 (2026-06-13)

- **저글링 모션 재건축**: 공식 파일 부재 확정(앱 번들 169파일·웹캐시 1005항목·CLI 바이너리·GitHub 전수 수색 — claude.ai가 코드로 렌더). Boss 화면 녹화(H.264)에서 재건축 — 파이프라인 진화: flood 키잉 → 연결성분 잔상 제거 → 아트 격자 양자화(pitch 3.9px 실측, 위상 프레임별 전수 탐색) → 눈 블롭 검출·2×2칸 정규화 → 투톤 음영(명도 분포 실측) → 합성 체커볼 도장. `tools/make_sprites.swift`.
- **공식 원본 위젯**: `clawd-magnifier.gif`(113프레임 무손실) → **탐정 돋보기 루프 위젯**. `tools/key_motion.swift`(흰배경 flood 키잉 + 전 프레임 공통 bbox 크롭 + 테라코타→인디고 리컬러). 케이블 모션(clawd-laptop.mov)은 동일 도구로 즉시 위젯화 가능 — 미사용 보류.
- **커뮤니티 라이브러리 시도**: clawd-on-desk(AGPL)의 Clawd GIF 24종 다운로드·8종 가공·우클릭 모션 전환 메뉴까지 구현 → **Boss 결정으로 전부 제거** ("내가 만든 게 아니라서 별로") — 에셋 삭제, 메뉴 제거.
- **최종 구성: 위젯 3개** — ① Clawd 집(생활 사이클: 대기→출근→타자→꾸벅→취침→복귀) ② Cody 집(동일, 비동기) ③ Clawd 탐정(공식 113f 루프, 11.3s). 캐릭터 0.48 스케일, 박자 0.1s. 저글링 재건축 스프라이트는 `assets/sprites/juggle`에 미사용 보존.
